...
if(!kanal) return message.channel.send(`mute log kanalı ayarlı değil.`)
  ...
  if (!message.member.roles.has(`${yetkili.id}`)) return message.channel.send(`**${ayarlar.prefix}mute** isimli komutu kullanabilmek için ${yetkili} rolüne sahip olman gerekiyor.`)
...
if(!sebep) sebep = 'Sebep belirtilmemiş.'
  ...
  .setColor(`#f3c7e1`)
  .setDescription(`Kime küfür ettiysen git özür dile, özür dilemek erdemliktir!`)
