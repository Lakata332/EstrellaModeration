...
if(!kanal) return message.channel.send(`Jail log kanalı ayarlı değil.`)
  ...
  let zaman = args[1]
  if(!args[1]) return message.channel.send(`Ne Kadar Jailde Duracağını Belirtmelisn!`)
...
if(!sebep) sebep = 'Sebep belirtilmemiş.'
  ...
  .setColor(`#f3c7e1`)
  .setDescription(`Al işte! Gene Birisi Bi Bok Yapmış!.`)
...
db.set(`${message.guild.id}.jail.${kişi.id}.roles.${r.id}`, r.id )})
    db.set(`${message.guild.id}.jail.${kişi.id}`, 'Megant')
    kanal.send(wasted)
